...
public:
	...
	bool intersect(ray& r, isect& i) const;
	//bool intersectKdTree(ray& r, isect& i, KdTree<Geometry>* currentNode, double t_min, double t_max) const;
	void intersectKdTree(ray& r, isect& i, KdTree<Geometry>* currentNode, bool& have_one, double t_min, double t_max) const;

	//bool intersectKdMain(ray& r, isect& i) const;
	...
	auto beginBoundedObjects() const { return boundedObjects.cbegin(); }
	auto endBoundedObjects() const { return boundedObjects.cend(); }
	const auto& getAllBoundedObjects() const {return boundedObjects; }
	...
	void buildKdTree(int depth, int leaf_size);
	void buildKdTrimesh(Geometry* trimesh, int depth, int leaf_size);
	void buildKdDetail(KdTree<Geometry>* kd_tree, int depth, int leaf_size, std::vector<std::vector<std::pair<Geometry*, int>>> orderedPlanes);

private:
	std::vector<std::unique_ptr<Geometry>> objects;
	std::vector<std::unique_ptr<Light>> lights;
	std::vector<std::unique_ptr<Geometry>> boundedObjects;
	std::vector<std::unique_ptr<Geometry>> nonboundedObjects;
	Camera camera;
...
struct KdStack{
    ...
    KdStack(KdTree<Geometry>* current_node, double t_min, double t_max){
